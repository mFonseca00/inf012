...
const appointmentService = {
  getMyAppointments: async (page = 0, size = 10, status = "", role = null) => {
    let url = `/appointment/my-appointments?page=${page}&size=${size}&sort=appointmentDate,asc`;
  ...
  getAll: async (page = 0, size = 10, status = "") => {
    let url = `/appointment/all?page=${page}&size=${size}&sort=appointmentDate,asc`;
    if (status) url += `&status=${status}`;
    const response = await api.get(url);
    return response.data;
  },

  scheduleAppointment: async (patientId, doctorId, appointmentDate) => {
    const response = await api.post("/appointment/schedule", {
      patientId,
